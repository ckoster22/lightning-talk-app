...
update msg page data modifier =
    case msg of
        ...
        NavigateTo route ->
            Show page data modifier ! [ Navigation.newUrl route ]
        ...
        _ ->
            Show page data modifier ! [ Cmd.none ]
...
handleCancel page data =
    if page == PreviousTalks then
        Show PreviousTalks data WithNoSelection ! [ Cmd.none ]
    else
...
handleCreateRoundFail page data modifier httpError =
    case ( page, modifier ) of
        ( Admin, WithRound formModel ) ->
            Show Admin data (WithRoundFormError formModel httpError) ! [ Cmd.none ]
...
handleCreateRoundSubmit page data modifier =
    case ( page, modifier ) of
        ( Admin, WithRound ( theme, startTime ) ) ->
            ...
            in
                Show page data modifier ! [ createRound round ]
...
handleCreateRoundSuccess data newRound =
    ...
    in
        Show UpcomingTalks nextData WithNoSelection ! [ Cmd.none ]
...
handleCreateTalkFail page data modifier httpError =
    case modifier of
        WithTalkSubmitting formModel formType _ ->
            Show page data (WithTalkFormError formModel formType (AsyncError httpError)) ! [ Cmd.none ]
    ...
    in
        case ( page, modifier ) of
            ( CreateEditTalkForm, WithTalkSubmitting _ _ _ ) ->
                Show UpcomingTalks nextData WithNoSelection ! [ Navigation.newUrl "/#upcoming" ]

            _ ->
                Show page nextData modifier ! [ Cmd.none ]
...
handleDeleteTalkFail page data modifier httpError =
    case modifier of
        WithDeletingPrompt talk _ ->
            Show page data (WithDeletePromptError talk httpError) ! [ Cmd.none ]
            ...
            let
                ...
                nextRound =
                    if round.slot1 == Just deleteFormModel.talk then
                        ...
                        round
            in
                Show UpcomingTalks data (WithDeletingPrompt deleteFormModel Nothing) ! [ deleteTalk nextRound ]
            ...
            let
                ...
                nextRound =
                    if round.slot1 == Just deleteFormModel.talk then
                        ...
                        round
            in
                Show UpcomingTalks data (WithDeletingPrompt deleteFormModel (Just httpError)) ! [ deleteTalk nextRound ]
...
handleGoToCreateForm data maybeTalkIdentifier =
    ...
    in
        Show CreateEditTalkForm data (WithTalk formModel Create) ! [ Cmd.none ]
    ...
    in
        case maybeEditTalk of
            Just talk ->
                ...
                in
                    Show CreateEditTalkForm data (WithTalk formModel Edit) ! [ Cmd.none ]
            ...
            Nothing ->
                ...
                in
                    Show CreateEditTalkForm data (WithTalkFormError formModel Edit TalkNotFoundError) ! [ Cmd.none ]
...
handlePromptForTalkDeletion page data modifier =
    case modifier of
        WithTimeslotSelected selectedTimeslot ->
            ...
            in
                Show UpcomingTalks data (WithDeletePrompt { talk = selectedLightningTalk, round = selectedTimeslot.round }) ! [ Cmd.none ]
...
handleSelectTimeslot timeslot page data =
    if page == UpcomingTalks then
        Show UpcomingTalks data (WithTimeslotSelected timeslot) ! [ Cmd.none ]
    else
        Show PreviousTalks data (WithTimeslotSelected timeslot) ! [ Cmd.none ]


handleSetRoundIsEditing : Page -> Data -> Modifier -> Round.Model -> ( Model, Cmd Msg )
handleSetRoundIsEditing page data modifier round =
    if page == UpcomingTalks then
        Show page data (WithRoundEditing round) ! [ Cmd.none ]
    else
        Show page data modifier ! [ Cmd.none ]
...
handleUpdateRound page data updatedRound =
    Show page data (WithRoundEditing updatedRound) ! [ Cmd.none ]
...
handleUpdateRoundCreateFormModel page data modifier formModel =
    case ( page, modifier ) of
        ( Admin, WithRound _ ) ->
            Show Admin data (WithRound formModel) ! [ Cmd.none ]

        ( Admin, WithRoundFormError _ error ) ->
            Show Admin data (WithRoundFormError formModel error) ! [ Cmd.none ]
...
handleUpdateRoundSubmit page data modifier =
    case modifier of
        WithRoundEditing round ->
            Show page data modifier ! [ updateRound round ]
        ...
        WithRoundEditing _ ->
            let
                ...
                nextData =
                    { data | rounds = nextRounds }
            in
                Show page nextData WithNoSelection ! [ Cmd.none ]
...
handleTalkSubmit data talkFormModel formType =
    ...
    in
        case maybeFormError of
            Just formError ->
                Show CreateEditTalkForm data (WithTalkFormError talkFormModel formType formError) ! [ Cmd.none ]
                    ...
                    cmd =
                        case talkFormModel.selectedRound of
                            ...
                            Nothing ->
                                Cmd.none
                in
                    Show CreateEditTalkForm data (WithTalkSubmitting talkFormModel formType Nothing) ! [ cmd ]
...
handleUpdateTalkFormModel page data modifier updatedTalkFormModel =
    case modifier of
        WithTalk _ formType ->
            Show page data (WithTalk updatedTalkFormModel formType) ! [ Cmd.none ]

        WithTalkFormError _ formType formError ->
            Show page data (WithTalkFormError updatedTalkFormModel formType formError) ! [ Cmd.none ]
